...
# -*- coding: utf-8 -*-
"""
RMT-compliant TR-based TensorHyper-VQC for quantum-dot classification.

This implementation enforces two separate input paths:

1. Gaussian latent input to the Tensor-Ring hypernetwork

       z ~ N(0, I)
       z -> TensorRingLayer -> VQC rotation parameters
...
The image is NEVER passed to the TR layer, and the Gaussian latent is NEVER
...
def validate_configuration(args) -> None:
    tr_input_size = math.prod(args.tr_input_dim)
    tr_output_size = math.prod(args.tr_output_dim)
    ...
    if len(args.tr_input_dim) == 0:
        raise ValueError("tr_input_dim cannot be empty.")

    if len(args.tr_output_dim) != len(args.tr_input_dim):
        raise ValueError(
            "tr_input_dim and tr_output_dim must have the same order."
        )

    if len(args.tr_ranks) != len(args.tr_input_dim):
        raise ValueError(
            "For Tensor Ring, tr_ranks must contain one cyclic rank per core."
        )

    if any(rank <= 0 for rank in args.tr_ranks):
        raise ValueError("All Tensor-Ring ranks must be positive.")

    if tr_output_size != required_output_size:
        raise ValueError(
            f"TR output size is {tr_output_size}, but the VQC requires "
            f"{required_output_size} = depth_vqc × num_qubits × 3. "
            "Change --tr_output_dim or the VQC dimensions."
        )
...
# =============================================================================
# Tensor-Ring hypernetwork
# =============================================================================
class TensorRingLayer(nn.Module):
    """
    Genuine cyclic Tensor-Ring linear map from Gaussian latent vectors to VQC parameters.
    ...
    def __init__(
        ...
        if len(tr_ranks) != order:
            raise ValueError(
                "tr_ranks must have exactly one cyclic rank per core."
            )
        ...
        for mode in range(order):
            ...
            rank_right = tr_ranks[(mode + 1) % order]
    ...
    def forward(self, gaussian_latent: torch.Tensor) -> torch.Tensor:
        if gaussian_latent.dim() != 2:
            raise ValueError(
                "Gaussian TR input must have shape [batch, latent_dim]."
            )

        if gaussian_latent.size(1) != self.input_size:
            raise ValueError(
                f"TR expected Gaussian latent dimension {self.input_size}, "
        ...
        required_symbols = 3 * order

        if required_symbols > len(symbols):
            raise ValueError(
                "TR order is too large for the einsum symbol implementation."
            )

        input_symbols = symbols[:order]
        output_symbols = symbols[order:2 * order]
        rank_symbols = symbols[2 * order:3 * order]
...
# =============================================================================
# TR-based TensorHyper-VQC wrapper
# =============================================================================
class TRTensorHyperVQC(nn.Module):
    """
    ...
    def __init__(
        self,
        tr_input_dims: List[int],
        tr_output_dims: List[int],
        tr_ranks: List[int],
        n_wires: int,
        n_qlayers: int,
        topology: str,
        ...
        self.tr = TensorRingLayer(
            input_dims=tr_input_dims,
            output_dims=tr_output_dims,
            tr_ranks=tr_ranks,
        )
    ...
    def forward(
        ...
        # Requirement 1: only Gaussian noise is passed to the TR layer.
        generated_angles = self.tr(
            gaussian_latents
        ).reshape(
            batch_size,
            self.vqc.n_qlayers,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "RMT-compliant TR-based TensorHyper-VQC with Gaussian TR input "
    ...
    parser.add_argument(
        ...
        default="5,10,5,10",
        help=(
            "TR input modes. Their product is the Gaussian latent dimension."
        ),
    )
    ...
    parser.add_argument(
        ...
        default="4,2,3,9",
        help=(
            "TR output modes. Product must equal depth_vqc*num_qubits*3."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Disable residual trainable global angles and use only TR-generated "
            "angles."
        ),
    )
    ...
    model = TRTensorHyperVQC(
        tr_input_dims=args.tr_input_dim,
        tr_output_dims=args.tr_output_dim,
        tr_ranks=args.tr_ranks,
    ...
    print("Architecture: RMT-compliant TR-based TensorHyper-VQC")
    print("TR input: Gaussian latent only")
    print(
        "Gaussian mode:",
        args.gaussian_mode,
    )
    ...
    checkpoint_path = os.path.join(
        args.save_path,
        f"best_tr_rmt_{args.model_kind}.pt",
    )
